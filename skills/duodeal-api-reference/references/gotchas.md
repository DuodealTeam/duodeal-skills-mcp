...
| A new quotation reads as broken: no status, no price category | `create_quotation` leaves `status_id` and `price_category_id` null | Set them right after creating: `update_quotation {id, status_id, price_category_id}`, with `list_quotation_statuses` (Draft) and `list_price_categories` (the default one) |
| A deal built through the connector is missing from the app's deals table (its client link works) | `create_quotation` leaves the quotation's `primaryQuotation` at `null`, and the table lists deals through their primary quotation | `PUT /quotations/{id} {"primaryQuotation": true}` by REST if a key is already configured, then check `get_deal` → `quotations[].primaryQuotation` is `true`; otherwise tell the user it is pending (Duodeal interface or the Duodeal team) |
