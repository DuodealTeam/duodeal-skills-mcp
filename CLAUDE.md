...
6. **Check how old these skills are.** They are a **copy** of the public repo with no git remote behind it, so an install silently stays on the version of the day it was made — one client ran five weeks on skills we had already fixed. A refresh normally happens on its own: `.claude/duodeal-skills-update.sh`, run by a `SessionStart` **and** a `UserPromptSubmit` hook, re-copies past 7 days and warns when it cannot. You still check, because the automation is exactly what an old install lacks: read the line `Skills Duodeal : mises à jour le <date>` in `DUODEAL-CONTEXT.md` (and `.claude/.duodeal-skills-stamp`, which the script keeps). **Absent, or more than 7 days old → offer the refresh in one sentence before working.** The procedure is **[`install/PROCEDURE.md`](https://github.com/DuodealTeam/duodeal-skills-mcp/blob/main/install/PROCEDURE.md)** in the public repo — follow it as written rather than improvising a shorter copy: install and refresh are the same steps, and the one everybody drops is putting the two hooks in place, which is what leaves a client frozen. Ask once per session, never refresh silently, and if the user declines, work with what is installed and drop it.
...
- **`accept` and `signstamp` always ship as a pair**: once signed, `accept` disappears and `signstamp` is the only proof of signature left. The newer `signature` block carries its own stamp once signed: it ships alone.
- **Primary quotation: check it on every quotation you create.** A quotation created with `create_quotation` (connector) or `POST /quotations` (REST) is born with `primaryQuotation: null`, even as the deal's first and only one (the deal's `primaryQuotationId` is set, the quotation's own flag is not). The app's deals table lists deals through their primary quotation, so such a deal is **missing from the table**, although it exists and its client link works. After `create_quotation`, read `get_deal` → `quotations[].primaryQuotation`: if it is not `true`, set it by REST (`PUT /quotations/{id} {"primaryQuotation": true}`) when a key is already configured, then re-read; otherwise tell the user before delivering. "Primary by default" is only guaranteed for deals created in the app: check, never assume. Procedure: **duodeal-quote-building** §5.
...
- **The PDF exports in A4 with 10 mm margins, no section cut in two.** Left alone the export is US Letter with no margins. The fix is one `<style>` (`@page` + keep-whole rules) in the `title` of the **first `lineType:"title"` line**, written with `update_quotation_line`: a line title renders in the root document, an html block does not, so this is not an exception to inline-first. Copy the exact rule and verify the page size (595 × 842 pt) as in **duodeal-mcp-best-practices** → PDF export.
